...
if _cc_role == "nv":
    ...
    _opts = [PAGE_OPS, PAGE_SYNC, PAGE_PRODUCTION, PAGE_PRICE, PAGE_TTKH, PAGE_QRSHOP, PAGE_QLCC]
    _default = PAGE_OPS
else:
    _opts = [PAGE_OPS, PAGE_PRODUCTION, PAGE_PRICE, PAGE_TTKH]
    _default = PAGE_OPS
if _is_owner:                               # chủ shop + zenzen197: thêm trang Tổng quan điều hành
    _opts = [PAGE_OVERVIEW, PAGE_SYNC] + [p for p in _opts if p != PAGE_SYNC]
...
def _render_data_sync_page():
    st.title("📥 Đồng bộ dữ liệu")
    st.caption("Bấm đồng bộ để tải số liệu một lần vào cache chung của app. Các trang/tab khác đọc lại cache này trong thời gian còn hạn, nên người mở sau sẽ nhanh hơn.")
    if not credential_present():
        st.warning("Chưa có kết nối Sapo LIVE nên không đồng bộ được.")
        return

    st.info(
        "Cache dùng chung cho các phiên đang chạy trên cùng app Streamlit. "
        "Nếu app bị restart/deploy lại thì cần bấm đồng bộ lại; bước sau có thể nâng thành snapshot lưu bền."
    )
    fresh = st.checkbox("Tải mới từ API trước khi làm nóng cache", value=True,
                        help="Bật mục này khi chị muốn số liệu mới. Nếu tắt, nút đồng bộ chỉ kiểm tra/làm nóng lại cache sẵn có.")
    ...
    def _run_tasks(tasks):
        rows = []
        bar = st.progress(0, text="Bắt đầu đồng bộ...")
        for idx, (label, fn, clear_fn) in enumerate(tasks, start=1):
            ...
            bar.progress(idx / max(len(tasks), 1), text=f"Đã đồng bộ {idx}/{len(tasks)}")
        bar.empty()
        st.dataframe(pd.DataFrame(rows), hide_index=True, width="stretch")

    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("⚡ Đồng bộ nhanh vận hành", width="stretch", type="primary"):
            _run_tasks([
                ("Cảnh báo chung", load_alerts, load_alerts.clear),
                ("Phiếu nhặt hàng", load_picking, load_picking.clear),
                ("Báo cáo cuối ngày", lambda: load_daily_report(None), load_daily_report.clear),
    ...
    with c2:
        if st.button("🧵 Đồng bộ sản phẩm/tồn kho", width="stretch"):
            _run_tasks([
                ("Danh mục + tồn kho Sapo", lambda: load_sapo_catalog(80), load_sapo_catalog.clear),
            ])
    with c3:
        if st.button("🐢 Quét đơn trả bị đóng cả năm", width="stretch"):
            _run_tasks([
                ("Đơn trả bị đóng cả năm", load_closed_returns_full_year, load_closed_returns_full_year.clear),
            ])

    with st.expander("ℹ️ Cách dùng cho nhiều người", expanded=False):
        st.markdown(
            "- Chị hoặc admin bấm **Đồng bộ nhanh vận hành** đầu ca.\n"
            "- Nhân viên mở tab nào thì tab đó đọc lại cache đã có, không phải ai cũng kéo lại từ Sapo/Dohana.\n"
            "- Nút quét cả năm để riêng vì nặng; chỉ bấm khi cần cập nhật bảng đơn trả bị đóng."
        )
