    ...
    def _total_gap_badges(rows):
        ...
        for row in rows or []:
            ...
            if isinstance(audit_row, dict):
                ...
                if ret_gap > 0 and not stale:
                    totals["ret_missing"] += ret_gap
                elif ret_gap < 0 and not stale:
...
# Băng "Việc cần làm" nằm ĐẦU sidebar: tạo chỗ TRƯỚC radio, đổ nội dung sau (khi loaders sẵn sàng).
_todo_slot = st.sidebar.container()
...
@st.cache_data(ttl=600, show_spinner=False)   # cache 10' → đổi trang trong 10' = tức thì, không tải lại
def _todo_counts_snapshot(need_lech, need_pick):
    # CHỈ tải count mà role cần → kho khỏi tải bảng 30-ngày nặng, CSKH khỏi tải phiếu nhặt.
...
def _render_todo_banner(is_cskh, is_kho, is_boss, opts):
    ...
    if not (is_cskh or is_kho or is_boss):
        return
    try:
        _snap = _todo_counts_snapshot(bool(is_cskh or is_boss), bool(is_kho or is_boss))
    except Exception:
        return
    ...
    if is_cskh or is_boss:
        _n = _snap.get("lech")
        if _n:
            _items.append(("🔺", f"{_n} số lệch cần kiểm",
    ...
    st.markdown(
        '<div style="border:2px solid #f59e0b;background:#fffbeb;border-radius:9px;'
        'padding:7px 10px;margin:0 0 6px 0">'
        '<div style="font-weight:900;color:#92400e;font-size:.95rem">📋 Việc cần làm</div>'
        f'<div style="color:#a16207;font-size:.72rem;font-weight:600">{_who} · bấm để tới chỗ xử lý · tự mất khi xong</div>'
...
with _todo_slot:   # đổ vào ĐẦU sidebar — luôn hiện, di chuyển trang không mất
...
def _tax_shop_combo(t, shops):
    """MỘT biểu đồ 'đường kết hợp cột' theo GIAN HÀNG (gọn):
    CỘT = 🔮 dự đoán cả năm (màu theo ngưỡng) · ĐƯỜNG = thực tế đã đạt · 2 vạch ngang = ngưỡng 3 tỷ & 10 tỷ."""
    _SAN_AB = {"Tiktokshop": "TTS", "Shopee": "SP", "Lazada": "LZ"}

    def _short(nm):
        parts = nm.rsplit(" - ", 1)
        brand = parts[0].replace(" BOUTIQUE", "").replace(" - ", "-").strip()
        san = _SAN_AB.get(parts[1], parts[1]) if len(parts) == 2 else ""
        return f"{brand}<br>{san}" if san else brand
    t3, t10 = t["t3"] / 1e9, t["t10"] / 1e9
    labels = [_short(s["name"]) for s in shops]
    proj = [s["proj"] / 1e9 for s in shops]
    ytd = [s["ytd"] / 1e9 for s in shops]

    def _c(v):
        return "#d9574e" if v >= t10 else ("#e0932f" if v >= t3 else "#3f9e70")
    fig = go.Figure()
    fig.add_trace(go.Bar(x=labels, y=proj, marker_color=[_c(v) for v in proj],
                         name="🔮 Dự đoán cả năm", text=[f"{v:.1f}" for v in proj],
                         textposition="outside", cliponaxis=False,
                         hovertemplate="%{x}<br>Dự đoán %{y:.2f} tỷ<extra></extra>"))
    fig.add_trace(go.Scatter(x=labels, y=ytd, mode="lines+markers", name="Thực tế đã đạt",
                             line=dict(color="#16233f", width=2.5),
                             marker=dict(size=8, color="#16233f", line=dict(color="#fff", width=1)),
                             hovertemplate="%{x}<br>Thực tế %{y:.2f} tỷ<extra></extra>"))
    fig.add_hline(y=t3, line=dict(color="#d97706", width=1.5, dash="dot"),
                  annotation_text="ngưỡng 3 tỷ", annotation_position="top left")
    fig.add_hline(y=t10, line=dict(color="#dc2626", width=1.5, dash="dot"),
                  annotation_text="ngưỡng 10 tỷ", annotation_position="top left")
    fig.update_layout(height=300, margin=dict(l=6, r=6, t=28, b=6), plot_bgcolor="rgba(0,0,0,0)",
                      yaxis=dict(title="tỷ đồng", rangemode="tozero"), bargap=0.45,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0))
    return fig
...
def _render_tax_warning(t):
    ...
    if shops:
        st.markdown("**📊 Dự đoán & thực tế theo gian hàng** — cột = 🔮 dự đoán cả năm (màu theo ngưỡng) · "
                    "đường = thực tế đã đạt · 2 vạch ngang = ngưỡng 3 tỷ & 10 tỷ (cột vượt vạch = vượt ngưỡng).")
        st.plotly_chart(_tax_shop_combo(t, shops), width="stretch")
...
def _render_sales():
    ...
    if _stores:
        with st.expander("📋 Bảng số liệu theo gian hàng", expanded=True):
            ...
            def _lostcell(val, n, qty, cval, rval, fval):   # tổng mất (đỏ) · đơn·SP · tách từng loại (nhỏ dưới)
                return (f'<td style="background:#fef2f2"><div class="main" style="color:#dc2626">{(val or 0) / 1e6:.0f}tr</div>'
                        f'<div class="sub">{_vni(n)} đơn · {_vni(qty)} SP</div>'
                        f'<div class="sub" style="color:#b91c1c">❌{(cval or 0) / 1e6:.0f} · '
                        f'↩️{(rval or 0) / 1e6:.0f} · 🚫{(fval or 0) / 1e6:.0f} tr</div></td>')
            _rows = ""
            for s in _stores:
                ...
                _rows += (
                    ...
                    f'{_lostcell(_lv(s), _ln(s), _lq(s), s.get("cancel_val", 0), s.get("refund_val", 0), s.get("fail_val", 0))}</tr>')
            ...
            _totrow = (
                ...
                f'{_lostcell(sum(_lv(s) for s in _stores), sum(_ln(s) for s in _stores), sum(_lq(s) for s in _stores), _sum("cancel_val"), _sum("refund_val"), _sum("fail_val"))}</tr>')
            st.markdown(
        ...
        if not _rows:
            ...
            def _tbl(_part):
                ...
                for _r in _part:
                    ...
                    # DƯ/THIẾU: app TỰ TÍNH = Tồn Sapo − (Tồn đầu + Hoàn − Bán ra).
            ...
            _css = ("*{box-sizing:border-box}"
                    ...
                    ".wrap{max-width:277mm;margin:0 auto;padding-top:2mm}"
                    ".brk{page-break-before:always}"
                    ".hd{display:flex;justify-content:space-between;"
                    "align-items:flex-end;border-bottom:2.5px solid #111;padding-bottom:5px;margin-bottom:6px}"
                    ".ttl{font-size:19px;font-weight:800;letter-spacing:.3px;line-height:1.15}"
                    ".sub{font-size:12px;color:#444;margin-top:3px}.sub b{color:#111}"
                    ".meta{font-size:13px;color:#111;text-align:right;line-height:1.7;white-space:nowrap}"
                    ".foot{font-size:11px;color:#666;margin-top:4px;page-break-before:avoid;"
                    "break-before:avoid}.ncc{font-size:10px;color:#0a7a55}"
                    # KHÔNG khoá break-inside cho .cols/table: bảng cao hơn 1 trang sẽ bị ĐẨY
                    # nguyên khối sang trang sau → trang đầu trống trơn. Chỉ khoá theo TỪNG DÒNG.
                    ".cols{display:grid;grid-template-columns:1fr 1fr;gap:9px;align-items:start}"
                    ...
                    "col.c-sku{width:20%}col.c-num{width:10.5%}col.c-cnt{width:13%}"
                    "td.n{text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}"
                    "td.g{font-weight:800;background:#dfe6f3;text-align:left;font-size:13px;letter-spacing:.2px}"
                    "td.sapo{font-weight:800;font-size:14px;background:#eef4ff}"
                    "th.sapo{background:#dbe6fb;font-weight:800}"
                    "td.app{font-weight:800;font-size:14px;background:#f0fbf3}"
        ...
        try:
            ...
            if _can_match_clip and _bulk_return_moves:   # chuyển vị trí = khớp mã → chỉ kho/quản lý
                with st.expander(
                    ...
                    st.caption("Chỉ gồm mã Đóng hàng dư và trùng chính xác đơn hoàn thiếu video trong cùng ngày.")
                    _bulk_move_keys = [
                        (str(x.get("date") or ""), str(x.get("code") or "").strip())
                        for x in _bulk_return_moves
                        if str(x.get("date") or "") and str(x.get("code") or "").strip()
                    ]
                    _bulk_all_key = "select_all_valid_package_to_inbound_30d"

                    def _toggle_all_valid_return_moves():
                        _checked = bool(st.session_state.get(_bulk_all_key))
                        for _day, _code in _bulk_move_keys:
                            st.session_state[f"bulk_valid_return_move_{_day}_{_code}"] = _checked

                    st.checkbox(
                        f"☑️ Chọn tất cả {len(_bulk_move_keys)} mã",
                        key=_bulk_all_key,
                        on_change=_toggle_all_valid_return_moves,
                    )
                    _selected_bulk_moves = []
                    with st.form("bulk_valid_package_to_inbound_30d_form"):
                        _bulk_moves_by_day = {}
                        for _day, _code in _bulk_move_keys:
                            _bulk_moves_by_day.setdefault(_day, []).append(_code)
                        _stt = 0
                        for _day, _codes in _bulk_moves_by_day.items():
                            try:
                                _day_label = datetime.fromisoformat(_day).strftime("%d/%m")
                            except Exception:
                                _day_label = _day
                            st.markdown(f"**📅 {_day_label} — {len(_codes)} mã**")
                            _bulk_cols = st.columns(2)
                            for _day_stt, _code in enumerate(_codes):
                                _stt += 1
                                _package_url = _with_url_query(
                                    "https://dhn.io.vn/order/", q=_code, orderCode=_code
                                )
                                with _bulk_cols[_day_stt % 2]:
                                    if st.checkbox(
                                        f"[**{_stt}. {_code}**]({_package_url})",
                                        key=f"bulk_valid_return_move_{_day}_{_code}",
                                    ):
                                        _selected_bulk_moves.append((_day, _code))
                        _submit_bulk_moves = st.form_submit_button(
                            "✅ Chuyển các mã đã chọn sang Khui hoàn", use_container_width=True
                        )
                    if _submit_bulk_moves:
                        if not _selected_bulk_moves:
                            st.warning("Chưa chọn mã nào.")
                        else:
                            _saved = picklog.add_video_type_overrides([{
                                "date": _day,
                                ...
                                "type": "inbound",
                                "source_type": "package",
                                "reason": "approved_exact_return_match_in_30d_summary",
                            } for _day, _code in _selected_bulk_moves])
                            if _saved:
                                for _fn in (load_week_summary, load_dohana, load_dohana_inbound,
                                            load_dohana_date, load_dohana_inbound_date,
                                            load_dohana_videos, load_dohana_video_store):
                                    try:
                                        ...
                                        pass
                                st.success(f"Đã chuyển {len(_selected_bulk_moves)} mã sang Khui hoàn.")
                                st.rerun()
                            else:
                                st.error("Không lưu được. Vui lòng kiểm tra kho Gist.")
            else:
                st.caption("✅ Không có mã Đóng dư nào trùng đơn hoàn thiếu video trong 30 ngày.")

            _week_day_set = {
                str(x.get("iso") or "") for x in (_wk.get("days") or []) if str(x.get("iso") or "")
            }
            _bulk_undo_moves = [
            ...
            if _can_match_clip and _bulk_undo_moves:   # hoàn tác chuyển = khớp mã → chỉ kho/quản lý
                with st.expander(
                    ...
                    _bulk_undo_all_key = "select_all_undo_package_to_inbound_30d"
                    ...
                    st.checkbox(
                        ...
                        key=_bulk_undo_all_key,
                        on_change=_toggle_all_undo_moves_30d,
                    )
                    _selected_bulk_undo = []
                    ...
                    if _submit_bulk_undo:
                        if not _selected_bulk_undo:
                            ...
                            if _saved:
                                for _fn in (load_week_summary, load_dohana, load_dohana_inbound,
                                    ...
                                    try:
                                        ...
                                        pass
                                st.success(f"Đã hoàn tác {len(_selected_bulk_undo)} mã về Đóng hàng.")
                                st.rerun()
                            else:
