...
WEEK_SUMMARY_BUILD = "redeploy-fix-import-29jul"   # đổi mỗi lần sửa module → biết app đã reboot chưa
...
KN_DAYS = 3   # đơn trả ĐANG HOÀN từ N ngày trở đi → CẦN KN (user đổi 27/08/2026: quá 5 → quá hạn 3)
...
def get_returns_in_progress(fetch_json, max_pages: int = 120, canceled_max_pages: int = 120) -> dict:
    ...
    Cờ CẦN KHIẾU NẠI:
      ...
      • đang hoàn hàng (returning) QUÁ KN_DAYS ngày → khiếu nại
      • 'Trả hàng hoàn tiền' có mã hoàn về và đang hoàn quá KN_DAYS ngày → vẫn khiếu nại
      • NGOẠI LỆ: 'Trả hàng hoàn tiền' chỉ 1 VĐ và chưa quá KN_DAYS ngày → CHƯA khiếu nại
    (Giao hàng thất bại có 1 VĐ trả là bình thường; Trả hàng hoàn tiền phải có 2 VĐ.)
    Kèm SKU, SL SP, tổng tiền (total_price) mỗi đơn."""
    today = (_now_utc() + timedelta(hours=7)).date()
    _kn_days = KN_DAYS   # đang hoàn hàng QUÁ N ngày → CẦN KN (7 → 5 ngày 01/07 → 3 ngày 27/08)
    ...
    for x in inprog:
        ...
        if sstat == "no_return":
            ...
            complaint, reason = True, f"Đang hoàn hàng {age} ngày (quá hạn {_kn_days} ngày) — cần khiếu nại"
...
def _sales_fetch_range(fetch_json, start, end, max_pages=280):
    ...
    store_paid_qty = defaultdict(int)                     # SỐ SP đơn đã thanh toán / gian hàng
    truncated = False
        ...
        for p in range(1, 121):                          # ≤120 trang/tháng: page×250 ≤ 30.000 (giới hạn Sapo)
            ...
            for o in rows:
                ...
                if str(o.get("financial_status") or "").lower() == "paid":
                    ...
                    store_paid_qty[store] += sum(int(round(li.get("quantity") or 0))
                                                 for li in (o.get("line_items") or []))
    ...
    return {"total": total, "orders": orders_n, "by_month": dict(by_month),
            ...
            "store_cancelled_n": dict(store_cancelled_n),
            ...
            "store_paid_qty": dict(store_paid_qty),
...
def get_sales_analysis(fetch_json, period="thangnay", _v=None):
    ...
    for s in stores:
        ...
        s["placed"] = sp
        s["placed_val"] = cur.get("store_placed_val", {}).get(nm, 0.0)   # giá trị đơn đặt (gồm hủy)
        ...
        # ĐÃ NHẬN (về ví) = đơn financial_status=paid, TRỪ tiếp trả/hoàn/giao thất bại (tiền/đơn/SP THẬT giữ lại)
        _pv = cur.get("store_paid_val", {}).get(nm, 0.0)      # tiền đã thanh toán (gross)
        _pn = cur.get("store_paid_n", {}).get(nm, 0)          # đơn đã thanh toán (gross)
        _pq = cur.get("store_paid_qty", {}).get(nm, 0)        # SP đã thanh toán (gross)
        s["paid_val"] = max(0.0, _pv - (rv.get("return_and_refund", 0.0)
                                        + rv.get("refund", 0.0) + rv.get("delivery_failed", 0.0)))
        s["paid_n"] = max(0, _pn - _ret_ord)
        s["paid_qty"] = max(0, _pq - int(round(_ret_qty)))
