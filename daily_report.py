...
_CSS = """
  ...
  .clip-code{font-size:1.04em;}                                 /* mã clip khui nhỉnh hơn 1 chút cho dễ đọc — KHÔNG nở to */
...
def _express_detail(rep):
    ...
    # XẾP THEO THỜI GIAN trong ngày (gói → xuất kho): đọc theo trình tự công việc.
    # KHÔNG chia nhóm gian hàng nữa vì đã có cột Gian hàng riêng.
    def _key(d):
        return (str(d.get("packed_time") or "~"), str(d.get("issued_time") or "~"),
                str(d.get("name") or ""))
...
def report_html(rep, dv, now_str, sign_on="1", collapse_xot=True):
  ...
  <table class="return-table" style="table-layout:fixed;overflow-wrap:anywhere"><colgroup><col style="width:4%"><col style="width:23%"><col style="width:30%"><col style="width:22%"><col style="width:21%"></colgroup>{_thead}<tbody>{_recon_rows(_chunk, start=_starts[_si], clip_on=clip_on)}</tbody></table>
  {_tail}
